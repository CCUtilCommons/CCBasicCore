...
// =================== 主函数 ===================
int main() {
	const size_t N = 1000000; // 压力测试 100 万元素
	const size_t elem_size = sizeof(int);
	// List-backed deque
	CCDeque* listDeque = CCBasicCore_CCListDequeCreate(elem_size);
	test_deque_edge(listDeque, "ListDeque");
	listDeque = CCBasicCore_CCListDequeCreate(elem_size);
	stress_test_deque(N, listDeque, "ListDeque");
	listDeque = CCBasicCore_CCListDequeCreate(elem_size);
	random_ops_deque(N, listDeque, "ListDeque");
