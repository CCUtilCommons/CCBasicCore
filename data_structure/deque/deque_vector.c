...
static const char* _wrongoverride_vector_message = "Wrong Impl Error happens in CCVector Deque";
...
static void _pushFrontVectorPtr(
	...
	if (CCBasicCore_SUCCESS != __CCBasicCoreVector_PushFront(vector, elem, size)) {
...
static void _pushBackVectorPtr(
	...
	// Pass the Impl Check
	CCVector* vector = (CCVector*)queue->deque_impl;
	if (CCBasicCore_SUCCESS != __CCBasicCoreVector_PushBack(vector, elem, size)) {
		_FkOffTheWrongType(queue, "Error in Handling the Size Session");
		return;
	}
}
